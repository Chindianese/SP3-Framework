...
void RenderingManager::RenderPassMain(Scene* scene)
{
	...
	case CameraComponent::CAM_CUSTOM_PERSPECT:
		projection.SetToPerspective(CameraObject->GetComponent<CameraComponent>()->GetFOV(), (float)Application::GetWindowWidth() / (float)Application::GetWindowHeight(), 0.1f, 1000);
		break;
